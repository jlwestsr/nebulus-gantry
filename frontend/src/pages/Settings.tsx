...
function ChatOptionsSection() {
  const { showTimestamps, showTokenUsage, showGenerationSpeed, setPreference } =
    usePreferencesStore();

  const toggles: { key: 'showTimestamps' | 'showTokenUsage' | 'showGenerationSpeed'; label: string; description: string; value: boolean }[] = [
    {
      key: 'showTimestamps',
      label: 'Always show timestamps',
      description: 'Display message timestamps without hovering',
      value: showTimestamps,
    },
    {
      key: 'showTokenUsage',
      label: 'Show token usage',
      description: 'Display prompt and completion token counts on AI responses',
      value: showTokenUsage,
    },
    {
      key: 'showGenerationSpeed',
      label: 'Show generation speed',
      description: 'Display tokens per second on AI responses',
      value: showGenerationSpeed,
    },
  ];
        ...
        {toggles.map((toggle) => (
          <div key={toggle.key} className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={() => setPreference(toggle.key, !toggle.value)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                toggle.value ? 'bg-blue-600' : 'bg-gray-600'
              }`}
            >
              <span
