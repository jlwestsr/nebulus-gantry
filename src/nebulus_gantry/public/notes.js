...
Nebulus.Notes = {
        ...
        Object.keys(grouped).sort().forEach(cat => {
            ...
            if (isExpanded) {
                grouped[cat].forEach(note => {
                    ...
                    // Click handler for selection (filtered to ignore menu clicks via propagation stop)
                    ...
                    optionsBtn.style.top = '10px';
                    optionsBtn.onclick = (e) => this.showContextMenu(e, note.id);
                    ...
                    el.appendChild(optionsBtn); // Add button
                    el.appendChild(date);
    ...
    /* =========================================
       Context Menu
       ========================================= */
    showContextMenu: function (event, noteId) {
        event.stopPropagation();
        this.closeContextMenu();

        const menu = document.createElement('div');
        menu.className = 'context-menu';
        menu.id = 'active-note-context-menu';
        ...
        options.forEach(opt => {
            ...
            item.onclick = (e) => {
                e.stopPropagation();
                this.handleMenuAction(opt.action, noteId);
                this.closeContextMenu();
            };
        ...
        menu.style.left = `${rect.left - 100}px`; // Shift left to keep on screen inside sidebar
    },

    closeContextMenu: function () {
        const existing = document.getElementById('active-note-context-menu');
        if (existing) existing.remove();
    },

    handleMenuAction: function (action, noteId) {
        if (action === 'share') {
            const url = window.location.origin + '/notes?note_id=' + noteId;
            navigator.clipboard.writeText(url).then(() => {
                Nebulus.Utils.showToast('Note link copied to clipboard');
            }).catch(err => {
            ...
            Nebulus.Modal.prompt("Rename Note", "Enter a new title:", note.title, (newTitle) => {
                if (newTitle && newTitle.trim()) {
                    ...
                    // We have `note` from state.
                    const updatedData = { ...note, title: newTitle.trim() };
                    ...
                    fetch(`/api/notes/${noteId}`, {
                        ...
                        .then(data => {
                            ...
                            if (idx > -1) this.state.notes[idx] = data;
                            this.renderNotesList();

                            // If current note, update title input too
                            if (this.state.currentNoteId === noteId) {
                                if (this.dom.titleInput) this.dom.titleInput.value = data.title;
                            }
                        })
                        .catch(err => alert('Error renaming note'));
                }
            });
        } else if (action === 'delete') {
            // Reuse generic delete logic but for specific ID
            if (!confirm('Are you sure you want to delete this note?')) return;
